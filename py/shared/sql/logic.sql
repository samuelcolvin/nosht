...
CREATE OR REPLACE FUNCTION iso_ts(v TIMESTAMP) RETURNS VARCHAR(63) AS $$
  DECLARE
  BEGIN
...
CREATE OR REPLACE FUNCTION iso_ts_tz(v TIMESTAMPTZ, tz VARCHAR(63)) RETURNS VARCHAR(63) AS $$
  DECLARE
  BEGIN
...
CREATE OR REPLACE FUNCTION tz_abbrev(v TIMESTAMPTZ, tz VARCHAR(63)) RETURNS VARCHAR(63) AS $$
  DECLARE
  BEGIN
    PERFORM set_config('timezone', tz, true);
    return to_char(v, 'TZ');
